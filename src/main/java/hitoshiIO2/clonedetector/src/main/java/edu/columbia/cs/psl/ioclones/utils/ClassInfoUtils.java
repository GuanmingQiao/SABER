...
public class ClassInfoUtils {
    ...
    public static String[] genMethodKey(String owner, String name, String desc) {
        /***********NOT ACTUALLY USED ANYWHERE OTHER THAN ProjInfoCollector, WHICH IS UTILITY SCRIPT!!!************/
...
//        }
    ...
    public static String methodNameArgs(String name, String desc) {
        ...
        String returnString = parseType(returnType);
        ...
        return name + argString + returnString;
    }
    ...
    public static boolean shouldInstrument(String className) {
        // do not attempt to instrument classes not in the metadata db
        if (GlobalInfoRecorder.queryClassInfo(className) == null) {
            logger.debug("Class " + className + " not in metadata database; skipping instrumentation");
            return false;
        }
    ...
    public static boolean isWritable(String className) {
        ...
        while (queue.size() > 0) {
            ...
            if (ci == null) {
                logger.debug("No class info: " + curName);
                //System.exit(-1);
                return false;
            }
