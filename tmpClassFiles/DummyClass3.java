...
public class DummyClass3 implements Serializable {
    ...
    public DummyClass3(String firstName, String lastName, List<String> strings, double decimal) {
    ...
    public List<Integer> selectionSort() {
        for (int i = 0; i < values.size() - 1; i++) {
            int index = i;
            for (int j = i + 1; j < values.size(); j++) if (values.get(j) < values.get(index))
                index = j;
